...
{
    {{conv.child[0].type_info.lib.name}} const &{{conv.child[0].value_name.lib}} = {{conv.child[0].deref_pointer_prefix}}it->first;
    {{conv.child[0].render_lib_to_edk_decl() | indent(4)}}
    {{conv.child[1].type_info.lib.name}} const &{{conv.child[1].value_name.lib}} = {{conv.child[1].deref_pointer_prefix}}it->second;
